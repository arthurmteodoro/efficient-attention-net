...
#opt = tfa.optimizers.AdamW(weight_decay=1e-4, learning_rate=1e-3, decay=1e-3/40)
...
model_history = learner.fit(1e-3, 3, cycle_len=2, cycle_mult=4, class_weight=class_weight_data, callbacks=[ch1, ch2, ch3])
#model_history = model.fit(train_ds, steps_per_epoch=len(original_train_images)//BATCH_SIZE, epochs=40, class_weight=class_weight_data, validation_data=test_ds, callbacks=[ch1, ch2, ch3])
